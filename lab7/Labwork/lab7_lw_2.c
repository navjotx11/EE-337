...
unsigned char voltage[4];           // Variable for strong final voltage
float fvoltage=0;
...
void timer0_ISR (void) interrupt 1
{
	...
	if(count==10){
		fvoltage=tempVal;
		fvoltage/=10;                                        //Average Value
    //avgVal=(avgVal*3.3)/1024;
		fvoltage*=(float)3.22;
		avgVal=(int)fvoltage;
		voltage[0]=avgVal/1000;                            //Compute Individual voltage values
		voltage[1]=(avgVal-(voltage[0]*1000))/100;
		voltage[2]=(avgVal-(voltage[0]*1000)-(voltage[1]*100))/10;
		voltage[3]=avgVal-(voltage[0]*1000)-(voltage[1]*100)-(voltage[2]*10);
		voltage[0]+=48;
		voltage[1]+=48;
		voltage[2]+=48;
		voltage[3]+=48;
		sdelay(100);  
		LCD_CmdWrite(0x89);
		LCD_StringWrite(voltage,4);                       //write Voltage values to LCD			
		tempVal=0;
		count=0;
	}
	else{
